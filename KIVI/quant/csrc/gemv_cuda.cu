...
__global__ void bgemvn_kernel_outer_dim(
        const half* _inputs, const uint32_t* _weight, const half* _zeros, const half* _scale, half* _outputs,
        const int IC, const int OC, const int group_size, const int nh, const bool mqa, int bitn){
    // const int group_size = 64;
    const int bit = bitn;
    ...
    const bool mqa)
{
    ...
    if (bit == 4){
      ...
      bgemv2_kernel_outer_dim<<<num_blocks, num_threads>>>(
        ...
        bgemvn_kernel_outer_dim<<<num_blocks, num_threads>>>(
            // pointers
            in_feats, kernel, zeros, scaling_factors, out_feats,
            // constants
