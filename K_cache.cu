...
__global__ void k_cache_save_prefill_kernel(unsigned char* k_cache_first_8_data,
    ...
    for(int seq = block_start_seq; seq < block_end_seq; ++seq){
        ...
        k_cache_first_8_data[(blockIdx.x * stride_batch_k_cache + blockIdx.y * stride_kvhead_k_cache + blockIdx.z * stride_block_k_cache + threadIdx.x * stride_dim_k_cache) * 2 + first_8_index]
        ...
        k_cache_mid_4_data[blockIdx.x * stride_batch_k_cache + blockIdx.y * stride_kvhead_k_cache + blockIdx.z * stride_block_k_cache + threadIdx.x * stride_dim_k_cache + mid_4_index]
        ...
        k_cache_last_4_data[blockIdx.x * stride_batch_k_cache + blockIdx.y * stride_kvhead_k_cache + blockIdx.z * stride_block_k_cache + threadIdx.x * stride_dim_k_cache + mid_4_index]
...
void k_cache_save(torch::Tensor &k_cache_first_8,
    ...
    if (start_pos == 0){
        // prefill
        int use_block = (seqlen + d_block - 1) / d_block;
...
torch::Tensor k_cache_compute(torch::Tensor &k_cache_first_8,
    ...
    torch::Tensor s = torch::empty({bsz, n_kv_heads, 1, n_blocks * d_block * 2}, options);
    unsigned char* k_cache_first_8_data = k_cache_first_8.data_ptr<unsigned char>();
    unsigned char* k_cache_mid_4_data = k_cache_mid_4.data_ptr<unsigned char>();
    unsigned char* k_cache_last_4_data = k_cache_last_4.data_ptr<unsigned char>();
    ...
    int stride_kvhead_s = n_blocks * d_block;
